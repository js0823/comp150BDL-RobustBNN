...
def create_NN(n_hidden, mean, var, nn_input, nn_output, X_train, Y_train, conv=False):
	if conv is False: # Create BNN
		...
		with pm.Model() as neural_network:
			...
			act_1 = pm.math.tanh(pm.math.dot(nn_input, weights_in_1))
			act_2 = pm.math.tanh(pm.math.dot(act_1, weights_1_2))
			act_out = T.nnet.softmax(pm.math.dot(act_2, weights_2_out))
			
			out = pm.Categorical('out', act_out, observed=nn_input, total_size=Y_train.shape[0]) # IMPORTANT for minibatches
