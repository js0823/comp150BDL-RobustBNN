...
def mnist_tutorial_jsma(train_start=0, train_end=60000, test_start=0,
  ...
  ###########################################################################
  # Craft adversarial examples using the basic iterative method
  ###########################################################################
  ...
  if (ATTACK == "BasicIterativeMethod"):
    # Instantiate a BasicIterativeMethod attack object
    attack = BasicIterativeMethod(model, sess=sess)
    attack_params = {
      'eps': 0.18, #maximum distortion of adversarial examples compared to orig, default = 0.3
  ...
  print('Test accuracy on adversarial test examples: {0}'.format(acc))
